...
class AsyncEvaluator:
    ...
    has no knowledge of the rest of the SDK environment; if it needs to retrieve flags or segments
    that are referenced by a flag, it does so through a read-only async interface that is provided
    in the constructor. It also produces feature events as appropriate for any referenced prerequisite
    ...
    def __init__(
        ...
        """
        :param get_flag: async function provided by AsyncLDClient that takes a flag key and returns either the flag or None
        :param get_segment: same as get_flag but for segments
        :param get_big_segments_membership: async function that takes a context key (not a context hash) and returns a
            tuple of (membership, status) where membership is as defined in BigSegmentStore, and status is one
    ...
    async def _check_prerequisites(self, flag: FeatureFlag, context: Context, state: EvalResult, event_factory: EventFactory) -> Optional[dict]:
        ...
        try:
            ...
            for prereq in flag.prerequisites:
                ...
                if (prereq_key == state.original_flag_key
                        or (state.prereq_stack is not None and prereq.key in state.prereq_stack)):
    ...
    async def _big_segment_match_context(self, segment: Segment, context: Context, state: EvalResult) -> bool:
        ...
        if not has_cached_membership:
            if self.__get_big_segments_membership is None:
                state.big_segments_status = BigSegmentsStatus.NOT_CONFIGURED
                return False
            result = await self.__get_big_segments_membership(key)
