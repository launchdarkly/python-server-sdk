...
from ldclient.interfaces import UpdateProcessor, AsyncFeatureStore
from ldclient.versioned_data_kind import FEATURES, SEGMENTS

from ld_eventsource import AIOSSEClient
...
class AsyncStreamingUpdateProcessor(Thread, UpdateProcessor):
    def __init__(self, config, store: AsyncFeatureStore, ready, diagnostic_accumulator):
        Thread.__init__(self)
        self.daemon = True
        self._uri = config.stream_base_uri + STREAM_ALL_PATH
        self._config = config
        self._data_source_update_sink = config.data_source_update_sink
        # self._data_source_update_sink = None
        self._store = store
        self._running = False
        self._ready = ready
    ...
    def start(self):
        if self._streaming_task is None:
            self._streaming_task = asyncio.create_task(self.run())
    ...
    def _create_sse_client(self) -> AIOSSEClient:
    ...
    async def _process_message(self, store: AsyncFeatureStore, msg: Event) -> bool:
        if msg.event == 'put':
            ...
            target = AsyncStreamingUpdateProcessor._parse_path(path)
            if target is not None:
                await store.upsert(target.kind, obj)
            else:
            ...
            target = AsyncStreamingUpdateProcessor._parse_path(path)
            if target is not None:
                await store.delete(target.kind, target.key, version)
            else:
