...
async def test_all_flags_state_returns_flag_values():
    """all_flags_state() returns the values of all stored flags."""
    store = MockAsyncFeatureStore()
    # init() decodes to model objects, matching how a real data source populates
    # the store; all_flags_state reads via store.all() without decoding.
    ...
    assert values['flag-a'] == 'value-a'
    assert values['flag-b'] == 'value-b'


@pytest.mark.asyncio
async def test_all_flags_state_degrades_per_flag_on_evaluator_error():
    """A flag whose evaluation raises degrades only that flag.

    The loop must not raise UnboundLocalError when the first flag raises, and a
    failed flag must not inherit a previous good flag's prerequisites.
    """
    from ldclient.evaluation import EvaluationDetail

    store = MockAsyncFeatureStore()
    # Ordering matters: 'bad-first' raises on the first iteration (would
    # UnboundLocalError if result were read unconditionally), and 'bad-last'
    # raises after a good flag set result (would reuse the good result).
    await store.init({FEATURES: {
        'bad-first': _make_flag('bad-first', 'x'),
        'good': _make_flag('good', 'y'),
        'bad-last': _make_flag('bad-last', 'z'),
    }})
    ...
    async with AsyncLDClient(config) as client:
        ...
        good_result.detail = EvaluationDetail('y', 2, {'kind': 'FALLTHROUGH'})
        ...
        client._evaluator.evaluate = AsyncMock(side_effect=fake_evaluate)
    ...
    # The good flag carries its own prerequisites.
    assert metadata['good'].get('prerequisites') == ['prereq-of-good']
    # The failed flags must not inherit the good flag's prerequisites.
